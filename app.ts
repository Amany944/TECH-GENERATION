...
type AgeFilter = "all" | "6-9" | "10-13" | "14-19";

interface Parcours {
  id: Exclude<AgeFilter, "all">;
  title: string;
  description: string;
  outcomes: string[];
}
...
const typedParcours = parcours as Parcours[];
const typedProjets = projets as Projet[];
const typedRessources = ressources as Ressource[];

const ageFilters: Array<{ label: string; value: AgeFilter }> = [
...
const $ = <T extends HTMLElement = HTMLElement>(selector: string): T | null => document.querySelector(selector);
...
function updateFilterState(activeValue: AgeFilter): void {
  document.querySelectorAll<HTMLButtonElement>("#projectFilters button").forEach((button) => {
...
function renderProjets(age: AgeFilter = "all"): void {
  const grid = $("#projetsGrid");
  if (!grid) return;

  const visibleProjects = age === "all" ? typedProjets : typedProjets.filter((project) => project.age === age);
  clear(grid);
...
function renderRessources(): void {
  ...
  typedRessources.forEach((resource) => {
...
function validateAccount(account: AccountInput): string {
...
function showFeedback(element: HTMLElement, message: string, variant: "error" | "success"): void {
